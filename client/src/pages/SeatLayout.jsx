...
const SeatLayout = () => {
  ...
  const [occupiedSeats, setOccupirdSeats] = useState([]);
  ...
  const handleSeatClick = (seatId) => {
    ...
    if (occupiedSeats.includes(seatId)) {
      return toast("This Seat is alreay Booked");
    }
  ...
  const getoccupiedSeatss = async () => {
    try {
      const { data } = await axios.get(
        `/api/booking/Seats/${selectedTime.showId}`,
      );
      if (data.success) {
        setOccupirdSeats(data.occupiedSeats);
      } else {
  ...
  return show ? (
    <div className="flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50">
      ...
      <div className="relative flex-1 flex flex-col items-center max-md:mt-16">
        ...
        {/* for a and b seats row */}
        <div className="flex flex-col items-center mt-10 text-xs text-gray-300">
          <div className="grid grid-col-2 md:grid-cols-1 gap-8 md:gap-2 md-6">
            {groupRows[0].map((row) => renderSeats(row))}
          </div>

          {/* for c and d seats row */}

          <div className="grid grid-cols-2 gap-11 mt-5">
            {groupRows.slice(1).map((group, inx) => (
              <div key={inx}>{group.map((row) => renderSeats(row))}</div>
            ))}
          </div>
        </div>

        {/* button for proceed to checkout */}
        <button
          onClick={bookTickets}
